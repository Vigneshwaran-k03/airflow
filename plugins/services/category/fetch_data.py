...
def fetch_categories_chunked(
    ...
    """
    Export categories in chunks and write multiple Parquet files:
    output_dir/
        part-00000.parquet
        part-00001.parquet
        part-00002.parquet
    ...
    while True:
        ...
        print(f"[fetch_categories] wrote {df.height} rows → {out_path}")

        offset += chunk_size
        part += 1

    print(f"[fetch_categories] done, total rows: {total_rows}")
